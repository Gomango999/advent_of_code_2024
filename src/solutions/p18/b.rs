...
pub fn solve() {
    ...
    let bytes_needed = (0..=bytes.len())
        .find(|&num_bytes| {
            let byte_subset: Vec<_> = bytes.iter().cloned().take(num_bytes).collect();
            if shortest_path(ROOM_SIZE, &byte_subset).is_none() {
                return true;
            }
            bar.inc(1);
            false
